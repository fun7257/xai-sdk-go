...
func main() {
	...
	defer client.Close()
	c := client.Chat.Create(types.ModelGrok3, chat.WithMessages(chat.User("long context...")))
	cmp, err := c.Compact(context.Background())
	if err != nil {
		log.Fatal(err)
	}
	fmt.Println(cmp.ID(), cmp.DroppedMessageCount())
}
