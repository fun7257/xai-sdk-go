...
Official protos are synced from [xai-org/xai-proto](https://github.com/xai-org/xai-proto)
into `third_party/xai/api/v1/`. Generated Go lives in `xai/api/v1/` with:
...
Public xai-proto lags Python SDK v6 for some surfaces. This repo keeps **local residual**
patches for wire parity with Python (field numbers from Python descriptors where applicable):
...
```bash
make proto   # protoc + protoc-gen-go + protoc-gen-go-grpc
go test ./...
```
